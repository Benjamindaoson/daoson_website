...
test('imports Markdown and copies same-directory images to public note assets', async () => {
  ...
  const plan = await createImportPlan({ sourcePath, siteDir, metadata, base: '/daoson_website/knowledge/' })
  ...
  assert.match(plan.rewrittenMarkdown, /!\[Diagram\]\(\/daoson_website\/knowledge\/notes\/state-diagram\/diagram.png\)/)
...
test('rejects a conflicting target note without overwriting it', async () => {
  ...
  await assert.rejects(
    createImportPlan({ sourcePath, siteDir, metadata: conflictMetadata, base: '/daoson_website/knowledge/' }),
...
test('summarizes an import plan and keeps it local when preview and publishing are declined', async () => {
  ...
  const plan = await createImportPlan({ sourcePath: previewSource, siteDir, metadata: { ...metadata, title: 'Preview note' }, base: '/daoson_website/knowledge/' })
